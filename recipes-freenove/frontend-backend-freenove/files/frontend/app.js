...
function IrMotionPanel({ ws, disabled }) {
  ...
  const read = async () => {
    ...
    try {
      ...
      setOutput(JSON.stringify(res.data, null, 2));
      const stderr = res?.data?.result?.stderr?.trim() || "";
  ...
  return (
    <Card icon="👁️" title="Capteur PIR HC-SR501" badge="GPIO24">
      ...
      >
        Signal actif a l'etat haut. Apres mise sous tension, le capteur peut
        demander jusqu'a 60 s d'initialisation.
      </div>
      {sensorData && (
        <div className="output" style={{ marginTop: 0, marginBottom: 10 }}>
          Niveau brut: {sensorData.value}
          {"\n"}
          Etat: {sensorData.detected ? "HIGH / detection" : "LOW / repos"}
        </div>
      )}
      {lastError && <div className="output">{lastError}</div>}
      <div style={{ display: "flex", gap: 8 }}>
        <button className="btn" onClick={read} disabled={disabled}>
          Lire
        </button>
